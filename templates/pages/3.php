...
<form action="./index.php" method="post" enctype="multipart/form-data">
    ...
    <table>
                ...
                <?php foreach ($genders as $key => $gender): ?>
                    <div style="display: flex;">
                        <div>
                            ...
                            <input style="margin: 0;width: 50%" type="radio" value="<?= $gender ?>"
                                   id="gender_<?= $key ?>"
                                   name="gender" <?= $chosenGender === $gender ? 'checked' : '' ?>>
                        </div>
                    </div>
        ...
        <tr>
            <td>Улюблені ігри:</td>
            <td style="display: flex;margin-bottom: 10px;margin-top: 10px ">
                <?php foreach ($favGames as $key => $game): ?>
                <div>
                    <input type="checkbox" name="favGames[]" id="game_<?= $key ?>"
                           value="<?= $game ?>" <?= in_array($game, $chosenGames) ? 'checked' : '' ?>>
                    <label for="game_ <?= $key ?>"><?= $game ?> </label>
                </div>
